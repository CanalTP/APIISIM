...
class TestChangeTransferMaxDistance(unittest.TestCase):
    ...
    def test(self):
        for conf_file, ref_dump_file in [('test1_1.conf', 'db_dump1'), ('test1_2.conf', 'db_dump2'),
                                         ('test1_3.conf', 'db_dump3'), ('test1_4.conf', 'db_dump4')]:
            self.assertTrue(tests.calculate_and_check(TEST_DIR + conf_file, TEST_DIR + ref_dump_file),
                            msg="Database dump different than expected dump (%s)" % ref_dump_file)
